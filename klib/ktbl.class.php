...
abstract class ktbl extends kdb {
  ...
  private $_angle = null;
	...
	public function __construct($ids, $angle=false) {
  ...
  public static function gets($query=null,$params=array(),$options=array()) {

    $_angle = array();

    if (isset($options['angle']) && is_numeric($options['angle'])) {

      if (isset(kch::$angles[$options['angle']])) {
        $_angle = array( 'host' => $_KCH_ANGLES[$options['angle']]['host'], 'user' => DB_USER, 'pass' => DB_PASSWORD, 'db' => DB_DATABASE);
      }
    }
		...
		foreach ((new $class)->columns($class) as $field=>$column) {
			if (isset($column['Key']) && $column['Key'] == 'PRI') {
				$primarys[$field] = null;
			}
		}

		$query = self::i($_angle)->q('SELECT * FROM %f '.$query, $class, $params);
	...
	public function get($result=null) {
		...
		if ($result != null || $result = self::i($this->_angle)->q($query.$this->primarysQuery(), array_values($this->primary))->fetch()) {
	...
	public function save() {
		...
		if ($this->exists()) {
			...
			if (count($updates) > 0) {
				...
				return self::i($this->_angle)->q($query, $values);
  ...
  public function delete() {
    ...
    return self::i($this->_angle)->q($query, array_values($this->primary));
